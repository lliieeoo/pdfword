...
async function getConverter(): Promise<ConvertFn> {
  return (await import("../lib/services/wordToPdf")).convertWordToPdf;
}

export default function WordToPdfPage() {
  const { file, error, isDragging, setFile, setDragging, reset } =
    useFileUpload({ accept: "docx" });
  ...
  return (
    <>
      <Head>
        <title>Word 转 PDF - DocFlow 在线转换工具</title>
        <meta name="description" content="免费在线将 Word 文档转换为 PDF 格式。支持中文内容嵌入，无需上传。" />
      </Head>

      <div className="max-w-3xl mx-auto px-4 py-10">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Word 转 PDF</h1>
          <p className="text-gray-500">将 Word 文档转换为 PDF 格式</p>
        </div>
        ...
        {!file && !downloadUrl && (
          ...
          >
            ...
            <p className="text-gray-400 text-sm">支持 .doc 和 .docx 格式，最大 50MB</p>
          </div>
        )}
